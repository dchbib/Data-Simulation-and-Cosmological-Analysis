...
class Gradient:
	...
	def SeekOfParameters(self, zz, ZETA, zform, m_lim, m_app, redshift, M_0, alfa, bbeta, slist, clist, MM, Cov, lambda0_Model, omega0_Model, couleur, KS, Realdata, degree_of_Polyn, Ncoefficients_of_KcorrPolyn, Scanning_on_parameters): # fobs, ftheo are array type. If you use a number, we must change chi2 = sum(.....
		...
		if Scanning_on_parameters == 'lambda0_omega0':
			...
			def func(a, z, bet):     # a = All_models;  z = redhsift;  bet = Beta
				...
				for i in range(len(a)):
					...
					if Bounced_limit==0:
						...
						#WeightingManifold = WeightingManifold*ten_power_ZETA_virt
						...
						#ten_power_ZETA_virt = (10**(ZETA_virt/5.))**beta
						#w_k = (ApparentMagnitude(z, a[i][0],a[i][1], 0.0, Realdata, degree_of_Polyn, Ncoefficients_of_KcorrPolyn).Weightingfactor(z, beta)*ten_power_ZETA_virt)/sum((ApparentMagnitude(z, a[i][0],a[i][1], 0.0, Realdata, degree_of_Polyn, Ncoefficients_of_KcorrPolyn).Weightingfactor(z, beta)*ten_power_ZETA_virt))
						#ten_power_s_m = (10**((m_app - slist)/5.))**beta
						#ten_power_s_m = (10**((bbeta*clist)/5.))**beta
						#w_k = (ApparentMagnitude(z, a[i][0],a[i][1], 0.0, Realdata, degree_of_Polyn, Ncoefficients_of_KcorrPolyn).Weightingfactor(z, beta)*ten_power_s_m)/sum((ApparentMagnitude(z, a[i][0],a[i][1], 0.0, Realdata, degree_of_Polyn, Ncoefficients_of_KcorrPolyn).Weightingfactor(z, beta)*ten_power_s_m))
						
						#ten_power_c_m = (10**((m_app - clist)/5.))**beta
						ten_power_c_m = (10**((-alfa*slist)/5.))**beta
						w_k = (ApparentMagnitude(z, a[i][0],a[i][1], 0.0, Realdata, degree_of_Polyn, Ncoefficients_of_KcorrPolyn).Weightingfactor(z, beta)*ten_power_c_m)/sum((ApparentMagnitude(z, a[i][0],a[i][1], 0.0, Realdata, degree_of_Polyn, Ncoefficients_of_KcorrPolyn).Weightingfactor(z, beta)*ten_power_c_m))
						
						#COVA_riance = sum(w_k*(ZETA_tild - sum(w_k*ZETA_tild ))*(ZETA_virt - sum(w_k*ZETA_virt)))
						#COR_elation = (1./(sqrt(sum(w_k*(ZETA_tild - sum(w_k*ZETA_tild))**2.0))*sqrt(sum(w_k*(ZETA_virt - sum(w_k*ZETA_virt))**2.0))))*COVA_riance
						#COVA_riance = sum(w_k*(slist - sum(w_k*slist ))*(m_app - sum(w_k*m_app)))
						...
						#COVA_riance = sum(w_k*(clist - sum(w_k*clist ))*(m_app - sum(w_k*m_app)))
			...
			def func(a, z, bet):     # a = All_models;  z = redhsift;  bet = Beta
				...
				for i in range(len(a)):
					...
					w_k = ApparentMagnitude(z, lambda0_Model, omega0_Model, 0.0, Realdata, degree_of_Polyn, Ncoefficients_of_KcorrPolyn).Weightingfactor(z, beta)/sum(ApparentMagnitude(z, lambda0_Model, omega0_Model, 0.0, Realdata, degree_of_Polyn, Ncoefficients_of_KcorrPolyn).Weightingfactor(z, beta))

					#ten_power_ZETA_virt = (10**(ZETA_virt/5.))**beta
					#w_k = (ApparentMagnitude(z, lambda0_Model, omega0_Model, 0.0, Realdata, degree_of_Polyn, Ncoefficients_of_KcorrPolyn).Weightingfactor(z, beta)*ten_power_ZETA_virt)/sum((ApparentMagnitude(z, lambda0_Model, omega0_Model, 0.0, Realdata, degree_of_Polyn, Ncoefficients_of_KcorrPolyn).Weightingfactor(z, beta)*ten_power_ZETA_virt))
					
					COVA_riance = sum(w_k*(Mlist_tild - sum(w_k*Mlist_tild ))*(m_app - sum(w_k*m_app)))
					#COR_elation = (1./(sqrt(sum(w_k*(Mlist_tild - sum(w_k*Mlist_tild))**2.0))*sqrt(sum(w_k*(m_app - sum(w_k*m_app))**2.0))))*COVA_riance
